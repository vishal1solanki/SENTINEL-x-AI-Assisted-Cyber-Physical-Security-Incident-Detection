...
def seed_database(db: Session = None):
    ...
    try:
        ...
        if db.query(User).first():
            print("[SEED] Database already seeded. Refreshing event timestamps to current live time...")
            now = datetime.now(timezone.utc)
            events = db.query(Event).order_by(Event.timestamp.asc()).all()
            if events:
                offset_minutes = 20.0
                step = offset_minutes / max(1, len(events))
                for i, ev in enumerate(events):
                    ev.timestamp = now - timedelta(minutes=(offset_minutes - (i * step)))
                db.commit()
                print(f"[SEED] Successfully updated {len(events)} event timestamps to live current time window.")
            return
